...
import { DocumentsPageClient } from "@/features/documents/DocumentsPageClient";
import { useTripLookup } from "@/features/trips/useTripLookup";

type DocumentsRouteClientProps = {
  tripId: string;
};

export function DocumentsRouteClient({ tripId }: DocumentsRouteClientProps) {
  ...
  if (!trip) {
    return (
      <RouteState
        ...
        description="Open eerst een bekende reis voordat je documenten bekijkt."
        actionLabel="Terug naar reizen"
      />
    );
  }

  return <DocumentsPageClient trip={trip} />;
}
